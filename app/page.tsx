          ...
          <div className="monitor-bezel">
            ...
            <div className="os-screen">
              ...
              <main className="os-workspace">
                ...
                {computerFile==="readme"&&<article className="os-window text-file readme-file"><header><span>{computerContent.readme.filename}</span><button onClick={() => setComputerFile("desktop")}>{siteContent.shared.minimize}</button></header><div><small>{computerContent.readme.meta}</small><h2>{computerContent.readme.title}</h2>{computerContent.readme.paragraphs.map((paragraph)=><p key={paragraph}>{paragraph}</p>)}<blockquote>{computerContent.readme.signature}</blockquote></div></article>}

                {computerFile==="lablog"&&<article className="os-window lab-log-window"><header><span>{computerContent.labLog.filename}</span><button onClick={() => setComputerFile("desktop")}>{siteContent.shared.minimize}</button></header><div className="lab-log-feed"><p className="lab-log-line lab-log-meta"><span>{computerContent.labLog.meta}</span></p>{computerContent.labLog.entries.map((entry)=><article className="lab-log-entry" key={`${entry.date}-${entry.title}`}><p className="lab-log-line lab-log-stamp"><span>[{entry.date}] {entry.type}</span></p><h2 className="lab-log-line"><span>{entry.title}</span></h2>{entry.body.map((paragraph)=><p className="lab-log-line" key={paragraph}><span>{paragraph}</span></p>)}<p className="lab-log-line lab-log-end" aria-hidden="true"><span>---</span></p></article>)}</div></article>}
