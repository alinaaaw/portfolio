...
- [ ] 盘点已经通过 GitHub Pages 或其他静态托管发布的小网页，为每个页面记录标题、简介、公开 URL、GitHub source URL、预览图、技术说明和当前可用状态，作为 Phase 6 站内项目入口的数据来源。
...
### 6.3 GitHub 小网页入口与 EMG 交互式 3D 展示

#### GitHub 已发布小网页入口

- [ ] 在 Computer 的 Projects 区域或一个独立的 Lab Apps 窗口中增加入口，集中展示已经发布的 GitHub Pages 小网页。
- [ ] 每个入口显示项目名称、简短说明、预览图、技术标签、最后确认日期，以及 **Open project** 和 **View source** 两个不同操作。
- [ ] 优先尝试在网站内部的 macOS 风格窗口中打开公开网页，让访客不离开 Lab 17 就能体验；同时保留“在新标签页打开”。
- [ ] 先检查目标网页的 `Content-Security-Policy` 和 `X-Frame-Options`。如果对方禁止 iframe 嵌入，站内窗口显示项目简介和预览图，再通过新标签页打开，不能留下空白或报错窗口。
- [ ] 对失效链接、加载超时和第三方网页不可用提供清楚的 fallback；外部内容不能阻塞 Lab 17 主体验。
- [ ] GitHub Pages URL 和 source repository URL 必须分开维护，避免把源代码页误当作可操作 demo。
- [ ] 小网页列表先使用本地 JSON 管理，不需要为了这个入口增加后端；以后有 CMS 时再接入统一内容管理。
...
完成标准：访客可以从 Lab 17 找到并可靠打开已发布的小网页，也可以在站内顺畅查看、操控并理解 EMG 模型；外部页面或 3D 加载失败时仍有完整的替代内容。首次加入这一整组互动展品通常作为 `MINOR` 版本发布；只有它取代原有核心探索方式时才考虑 `MAJOR`。
