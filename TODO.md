...
- [ ] 将重要数据自然写进项目叙事，说明数据代表什么、如何得到以及它如何支持结论，避免只堆数字。
- [ ] 检查数据来源、单位、时间范围和上下文；涉及公司、研究参与者或未公开成果的数据必须先确认可以公开。
- [ ] 新增公开事实后同步更新 dossier，保证未来 AI Bot 的回答与网站内容一致。
- [ ] 每完成一个项目的文字和数据补充，就按“持续 Commit / Push 规则”检查、commit 并 push；建议使用 `content: expand <project>`。
