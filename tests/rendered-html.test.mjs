...
test("source contains six room objects, real work, and a progressive reveal", async () => {
  ...
  assert.match(content, /prototype a small Lab Guide/);
  assert.match(content, /separate approved facts from working notes/);
  assert.match(content, /add a quiet feedback loop/);
  assert.match(content, /make Lab Log updates easier to publish/);
  assert.match(content, /polish the room one object at a time/);
  assert.match(content, /walk the whole room before release/);
