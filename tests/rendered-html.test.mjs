...
test("field case map uses real country geometry instead of hand-drawn continents", async () => {
  ...
  assert.match(mapReading, /preloadPhotoAspect/);
  ...
  assert.doesNotMatch(mapReading, /setPhotoLayout\(null\)/);
  assert.match(mapReading, /frameAspect:sourceAspect/);
  assert.match(mapReading, /frameWidth:Math\.min\(maxFrameWidth,maxFrameHeight\*sourceAspect\)/);
  assert.doesNotMatch(mapReading, /if\(sourceAspect>=1\)/);
  const pendingCheck=mapReading.indexOf("const pending=photoPreloadRef.current.get(photo.src)");
  const cachedCheck=mapReading.indexOf("const cachedAspect=photoAspectCacheRef.current.get(photo.src)");
  assert.ok(pendingCheck>=0&&cachedCheck>pendingCheck);
  ...
  assert.match(css, /\.field-map-memory \{[^}]*var\(--photo-frame-width,448px\) \+ 32px/);
  assert.match(css, /padding: 15px 15px 12px/);
  assert.match(css, /var\(--photo-frame-width,448px\) \+ 26px/);
