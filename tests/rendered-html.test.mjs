...
test("source contains six room objects, real work, and a progressive reveal", async () => {
  ...
  assert.match(content, /LAB 17 LAUNCH BOARD \/ TODO/);
  assert.match(content, /shape the next EMG iteration/);
  assert.match(content, /edit, verify, and simplify/);
  assert.match(content, /make Lab 17 the one official site/);
  assert.match(content, /refine the room and its interactions/);
  assert.match(content, /preview, listen, then release/);
  assert.match(content, /BOARD \/ TODO/);
