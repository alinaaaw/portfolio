...
test("field case map uses real country geometry instead of hand-drawn continents", async () => {
  ...
  assert.match(css, /\.field-map-memory \{[^}]*480px[^}]*100svh - 368px/);
  ...
  assert.match(css, /\.field-map-photo-stage \{[^}]*aspect-ratio: 1 \/ 1/);
  assert.match(css, /\.field-map-photo-stage \{[^}]*padding: 15px/);
  ...
  assert.match(css, /@media \(max-width: 620px\)[\s\S]*\.field-map-memory \{[^}]*474px[^}]*100svh - 222px/);
  assert.match(css, /@media \(max-width: 620px\)[\s\S]*\.field-map-photo-stage \{[^}]*padding: 12px/);
