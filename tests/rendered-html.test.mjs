...
test("portrait room, computer, and closeup sizing stay isolated from desktop architecture", async () => {
  ...
  assert.match(notebookPaperContract, /--portrait-notebook-rule: clamp\([\s\S]*width: min\(88vw,600px,calc\(\(100dvh - 96px\)\*\.79\)\)[\s\S]*max-width: none[\s\S]*aspect-ratio: \.79[\s\S]*background-repeat: no-repeat,repeat-y,no-repeat[\s\S]*background-size:[\s\S]*var\(--portrait-notebook-rule\)/, "portrait notebook papers must keep their established ratio at a compact responsive size while their rules scale continuously from the top edge");
