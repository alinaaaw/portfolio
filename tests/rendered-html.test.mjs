...
test("portrait room, computer, and closeup sizing stay isolated from desktop architecture", async () => {
  ...
  const bulletinMetaContract = declarationsFor(".portrait-notification-open small");
  ...
  assert.match(bulletinContract, /position: absolute[\s\S]*z-index: 20[\s\S]*border: 1px solid var\(--red\)/, "portrait bulletin must float above phone content without changing layout");
  ...
  assert.match(portraitComputer, /bulletin&&<aside className="portrait-phone-notification" role="status" aria-live="polite"/);
  assert.match(portraitComputer, /PhoneAppIcon kind="lablog" badge=\{bulletin\}/);
