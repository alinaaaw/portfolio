...
## 5.14 Portfolio disclosure decision — author confirmation (2026-08-27)

Wenrui confirmed the current public-portfolio boundary for this internship:

- The Thermo Fisher internship, the high-level role description, and the references to 3500Dx testing and Qantis/WinTrace workflow analysis may be named publicly.
- The Thermo Fisher portion must not publish concrete source code or company-specific data.
- The robot-automation project is outside that company-confidentiality restriction. Its current public portfolio summary and aggregate evidence may remain visible; the website does not publish source code.

This author confirmation supersedes the earlier **open portfolio-scope question** in this dossier for the current website copy. It does not authorize publication of customer data, unreleased interfaces, internal defect records, or any material outside the stated portfolio scope. If the internship scope or employer guidance changes, review this boundary again before adding new material.
...
| Thermo Fisher Internship | **Strong for robotics context; moderate for other workstreams** — automated laboratory tube handling and the remaining project contexts are clear | **Very strong for robotics** — dataset/model iteration, staged perception-to-action workflow, target reacquisition, physical calibration, reliability and safety reasoning; **strong at role level** for testing, workflow reconstruction, and onboarding | **Strong public portfolio scope for the robot-automation summary and aggregate evidence; company-specific source code and data remain excluded from the Thermo Fisher portion** | **Strong detector results** — latest four-class model: 0.977 precision, 0.974 recall, 0.926 mAP50–95 on 15 held-out images; **approximately 98% author-reported overall workflow success** during real-hardware testing; residual camera/hardware fault cause unresolved; other internship workstreams are not numerically quantified | **Context references plus local primary artifacts; see §5.14 for the current public-portfolio boundary** | **Strong robotics artifact set**; Miro workflow artifact; add visual assets only within the current public scope | Logged trial count and success definition, per-stage/cycle-time metrics, root cause of intermittent camera/hardware failures, controlled latency/robustness test, exact public hardware naming, and defect examples |
...
- retain the current Thermo Fisher disclosure decision in §5.14 and review it again before adding material beyond the present portfolio scope.
